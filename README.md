...
更新时间：2026-07-22
状态：工程默认已更新为3D-v2与三随机种子pooled ProtT5专家的OOF冻结联合模型；3D-v2仍是结构创新子模型

## 阅读导航与当前结论

- 项目完整实现了1D序列、2D原子图和3D预测结构三条线路；当前联合默认实际使用 **1D序列 + 3D结构** 两个维度，其中1D同时包含ESM2残基语义和pooled ProtT5全局先验。
- 默认结构专家3D-v2由ESM2序列Transformer、pLDDT-aware Cα-EGNN和全原子化学位点残差支路组成；联合默认以OOF冻结权重融合三种子ProtT5专家，最终权重为ProtT5/3D-v2=`0.62/0.38`、阈值`0.60`。
...
- 吸收新论文后的联合默认在留一折OOF上达到BACC/MCC/AUROC/AUPRC `0.9257/0.8666/0.9772/0.9535`；Test1为`0.9236/0.8588/0.9815/0.9649`，Test2为`0.8500/0.6456/0.9577/0.7851`。相对原3D-v2，两套历史测试的MCC和排序指标均提高，但它仍没有全面超过纯ProtT5性能候选或导师论文。
...
- 前沿3D路线已进入“真实构象集合”阶段：完成PepFlow可断点批处理、multi-model PDB转GHXTox构象缓存、接触占有率/RMSF/回转半径特征和fold-specific OOF等权评估入口，并固定了250条、30个fold×标签×拓扑分层的训练内pilot。当前本地没有PepFlow/BioEmu/AF3/Boltz新结构，因此尚无构象集合性能数值，默认模型不变。
...
3D-v2结构专家配置见 [`configs/default.json`](configs/default.json)，联合默认配置见[`configs/default_toxpltc_hybrid.json`](configs/default_toxpltc_hybrid.json)。结构专家在原3D-v1主干之外增加化学位点残差支路：
...
| ---: | --- | --- | --- |
| 1 | PepFlow全原子构象集合 | 每条肽生成8个构象；对每个构象复用3D-v2共享权重，固定做构象概率均值和ESMFold+PepFlow均值；并计算6/8/10 Å接触占有率、残基RMSF、构象RMSD、回转半径及成对距离均值/方差 | **代码、250条分层pilot和OOF协议已完成；等待服务器生成真实PDB** |
| 2 | BioEmu平衡态采样 | 作为独立构象来源验证PepFlow结论，并用于模型间/生成器间分歧；不把蛋白质单体上的论文性能直接外推到短肽 | 接口兼容，尚未生成；PepFlow pilot通过保留门槛后再投入 |
| 3 | AF3/Boltz跨模型静态共识 | 在同一pilot上与ESMFold比较接触图、官能团位置和分类概率分歧；用于结构可靠性，不把多次扩散sample直接称为热力学平衡态 | Boltz可作为开源入口，尚无本地缓存 |
| 4 | AfCycDesign/PepFlow约束采样 | 只对有实验或数据库拓扑标注的环肽/二硫键肽显式加入闭环或共价约束 | 已完成序列可见风险审计；缺少真实连接标签，暂不生成伪拓扑 |
| 5 | AlphaFold约束MD、MDGen或短程物理精修 | 只处理构象分歧高、序列/结构专家冲突或拟进入湿实验的少量样本 | 高成本后续项，不用于当前全量筛查 |

当前实际完成的拓扑审计见[`runs/peptide_topology_audit/summary.json`](runs/peptide_topology_audit/summary.json)。训练集6387条肽中，2795条长度为3–30 aa，可进入当前保守的PepFlow pilot范围；2588条至少含两个Cys，1459条达到预设高Cys风险规则。高Cys风险组的毒性正类比例很高，但这是标签相关和采样分布的统计关联，不能证明二硫键或成环导致毒性，也不能作为连接约束的替代标签。

第一轮pilot严格限制在训练数据内部：[`data/pepflow_pilot/train_pilot_250.fasta`](data/pepflow_pilot/train_pilot_250.fasta)由固定CD-HIT fold、标签和三档拓扑风险共同分层，覆盖30个组合；生成结构后，每条样本只由其held-out fold对应的3D-v2 checkpoint预测。评估只比较预先固定的单ESMFold、PepFlow等权均值和ESMFold+PepFlow等权均值，沿用阈值`0.677819`，不搜索构象权重或Test1/Test2阈值。只有完整pilot在MCC不下降的同时至少改善BACC、AUROC或AUPRC之一，才扩大到全部可支持训练样本；否则保留构象柔性分析，停止默认模型升级。

PepFlow或BioEmu生成的PDB没有ESMFold pLDDT。兼容性pilot默认沿用同一样本的ESMFold逐残基pLDDT，只让坐标、空间边和化学位点随构象变化；缓存同时标记`confidence_imputed=1`，防止把PDB B-factor误读为pLDDT。正式的ensemble模型应把生成器来源、构象分歧和物理质量作为独立可靠度输入，而不是伪造pLDDT。
...
本项目的贡献不在于重新发明ESM2、EGNN、原子图或跨注意力，而在于把毒性预测参考工作和最新多肽构象建模工作中分散的序列、原子化学、等变几何、多尺度建模、不平衡学习、构象集合与拓扑约束思路，组织成一个能够显式处理结构置信度、能够回退、也能被消融检验的框架。其中，“多化学位点—方向—连续双尺度相互作用残差”已经进入默认结构专家；“构象占有率—柔性—生成器分歧”是当前等待真实结构数据验证的下一层结构假说。
...
| 全原子构象集合与接触占有率 | PepFlow；BioEmu和MDGen作为后续独立来源 | PepFlow直接采样多肽全原子构象；BioEmu近似蛋白单体平衡分布；MDGen生成动力学轨迹 | 不把构象拆成独立训练样本；按肽组成bag，复用共享3D编码器并聚合构象概率，同时保存接触占有率、RMSF、回转半径和概率方差。当前完成真实数据入口与OOF pilot，未生成结构、未修改默认 |
...
本项目不复现PepFlow训练，只调用官方冻结参数生成构象。第一阶段固定每条肽8个构象，复用现有3D-v2编码器并做等权概率聚合，同时保存接触占有率、RMSF和回转半径。该路线当前处于“接口和OOF pilot已完成、真实构象待服务器生成”，不能写成已经提升指标。
...
| 3D-v2结构专家完整训练checkpoint | [`runs/3d_v2_default/best_model.pt`](runs/3d_v2_default/best_model.pt) |
...
| PepFlow训练内250条分层pilot | [`data/pepflow_pilot/train_pilot_250.fasta`](data/pepflow_pilot/train_pilot_250.fasta)、[`data/pepflow_pilot/train_pilot_250.csv`](data/pepflow_pilot/train_pilot_250.csv) |
...
论文可将项目定位为：**建立一个残基对齐的1D/2D/3D肽毒性比较与解释框架，在1D+3D联合默认中融合ToxPLTC启发的ProtT5全局专家与结构质量感知的全原子化学位点3D专家，并通过同源分组OOF、完整消融和能力感知回退说明预训练语义与结构信息的条件性互补。**
...
conda run -n dachuang_26 python -B -m ghxtox.pepflow_batch \
  --fasta data/pepflow_pilot/train_pilot_250.fasta \
...
| 3D-v2 + three-seed pooled ProtT5 hybrid default | 0.60 | 0.8588 | 0.9649 | 0.6733 | 0.6456 | 0.7851 | Current engineering default; OOF-frozen 0.38/0.62 structure/ProtT5 fusion |
...
The new three-seed ProtT5 TextCNN candidate exceeds the standalone 3D-v2 expert on test1/test2 MCC, F1, AUROC and AUPRC at its OOF-selected BACC-plateau threshold. OOF-frozen fusion of that expert with 3D-v2 is now the engineering default and raises standalone 3D-v2 ranking metrics on both historical tests, while pure ProtT5 remains the strongest same-data comprehensive candidate. The candidate remains about 0.0024 below the mentor ToxMSRC test2 MCC of 0.655, and no new pre-frozen external set has confirmed either upgrade.
...
- 已完成250条训练内pilot，按固定CD-HIT fold、标签和拓扑风险覆盖30个分层；只选择3–30 aa，避免在未验证长度上直接外推。
- 已实现官方PepFlow断点批处理、multi-model PDB解析、逐构象全原子化学位点缓存、6/8/10 Å接触占有率、残基RMSF、回转半径和距离分布统计。
- 已实现fold-specific OOF评估：每条肽只由其held-out fold checkpoint预测；只比较单ESMFold、PepFlow等权均值、ESMFold+PepFlow等权均值，不搜索权重或阈值。
- 本地还没有真实PepFlow/BioEmu/AF3/Boltz构象，因此当前没有新增预测指标，不修改默认模型，也不把dry-run描述成完成性能实验。
- 下一阻塞项是按第16节命令在AutoDL生成250×8个构象并回传压缩包。pilot不过门槛则停止；通过后才扩大训练构象并尝试set-attention聚合，随后才进入BioEmu和Boltz。

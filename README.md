...
更新时间：2026-07-17
状态：3D-v2 已升级为默认线路；配对统计、分层评估、论文案例归因和效率分析均已完成

## 阅读导航与当前结论

- 当前默认3D线路是“pLDDT-aware Cα-EGNN + 全原子化学位点交互残差”的3D-v2，OOF阈值为`0.677819`。
...
- 全原子化学位点支路经Pareto checkpoint选择后，在五个训练内同源分组fold上平均MCC/AUROC/AUPRC均为正增益；按项目“有提升即更新默认线路”的规则，已完成8轮全训练集重拟合并升级默认入口。
...
当前默认分类器是 **ESM2 + 序列 Transformer + pLDDT-aware Cα-EGNN + 全原子化学位点交互残差 + learned-confidence fusion** 的3D-v2。它在训练内五折中相对匹配3D-v1对照小幅提高MCC、F1、AUROC和AUPRC；完整重拟合后Test1的MCC/AUPRC也略有提高，但Test2的MCC/F1下降而AUPRC提高，说明收益偏向概率排序且仍具有分布依赖。严格ToxinPred3外部集目前只有历史3D-v1冻结结果，其中**不依赖结构的1D ESM2-BiLSTM迁移最好**。因此不能表述为“3D-v2全面优于所有模型”；1D仍是域外/无结构回退，3D-v1则是缺少化学位点张量时的工程回退。
...
| 单个位点 | 去掉每残基第二位点 | 0.8640 | -0.0037 | 0.9551 | +0.0000 | 拒绝 |
| 无疏水位点 | 去掉疏水通道及纯疏水位点 | 0.8658 | -0.0019 | 0.9557 | +0.0007 | 拒绝，未保住 MCC |
| 无方向 | 去掉方向不变量 | 0.8640 | -0.0037 | 0.9550 | -0.0001 | 拒绝 |
| 仅原始距离 | 去掉回转半径归一化 RBF | 0.8618 | -0.0059 | 0.9551 | +0.0000 | 拒绝 |
| 仅归一化距离 | 去掉原始 Å 距离 RBF | 0.8619 | -0.0058 | 0.9547 | -0.0004 | 拒绝 |
| 无相互作用类型 | 去掉六类显式配对通道 | 0.8620 | -0.0057 | 0.9556 | +0.0006 | 拒绝，未保住 MCC |
| 无 pLDDT | 去掉位点置信输入和边权 | 0.8620 | -0.0057 | 0.9553 | +0.0002 | 拒绝，未保住 MCC |
| 随机残差初始化 | 不使用零初始化 | 0.8658 | -0.0019 | 0.9551 | -0.0000 | 拒绝 |
| 加入同残基边 | 双位点可在残基内通信 | 0.8677 | 0 | 0.9554 | +0.0003 | 唯一晋级五折 |
...
按用户设定的“只要有提升就更新默认模型”规则，该候选已使用五折最佳epoch中位数8在全部6387条训练数据上重拟合，并替换项目默认3D入口。默认checkpoint为`runs/3d_v2_default/best_model.pt`，默认阈值为训练内OOF选择的`0.677819`。需要强调：这是项目工程默认升级，不等同于已经获得新的独立泛化证明；Test1/Test2/ToxinPred3均已被查看，下一次论文级升级判断仍必须使用预先冻结的新外部开发集或新的outer nested协议。
...
因此当前论文级结论应是：“多化学位点、方向和连续双尺度距离残差支路在五个训练内同源分组fold上相对冻结对照一致小幅改善MCC、F1和AUPRC；Pareto checkpoint选择纠正了MCC-only选择造成的排序损失，同残基位点边带来极小附加排序增益。该结构已升级为项目默认3D-v2，但其独立数据优势仍未确认。”若未来获得新的独立开发数据或预先批准的验证预算，可把侧链多构象稳定性、溶剂暴露和相互作用专属角度作为新研究假说；不得继续根据当前五折或已经查看的Test1、Test2、ToxinPred3调整位点定义和距离范围。
...
因此，OOF排序增益具有稳定的配对统计支持；OOF MCC以及Test1/Test2上的差异均不能拒绝“没有差异”。Test1/Test2标签在项目开发中已经被查看，其p值只能作为回顾性描述，不能作为新独立确认性检验。
...
当前三通路动态路由是在历史3D-v1基础上完成的，改善了校准和风险分层，但没有替换当前3D-v2。合理部署方式是同时报告：3D-v2概率、实际是否回退3D-v1、1D回退概率、结构质量和跨种子不确定性；当全原子化学位点不可得时先回退3D-v1，当结构整体不可得或域外偏移明显时优先参考1D。

---

## 7. 已实现的残基级生物学解释

解释模块现已同时覆盖历史3D-v1和当前3D-v2。3D-v2论文案例同时包含序列/ESM2归因、Cα空间诊断和化学位点专属删除干预：
...
项目建立了 group-aware OOF、nested group CV、三随机种子、bootstrap 区间、Brier/ECE 校准、pLDDT 分层和严格外部同源筛查。多尺度 CNN、ProtT5、SupCon、Focal BCE、高 pLDDT 子集训练、增强 2D 特征、局部坐标系、全主链几何、官能团多锚点双尺度图、残差专家、动态路由、选择性风险和 conformal 等未达到门槛的路线均被记录或停止，没有只保留正结果。化学位点支路进一步完成九项内部组件消融和 RankNet 负对照；无提升 checkpoint 已清理，只保留机器可读汇总以防重复。

### 8.6 创新六：把项目原创结构假说也纳入可证伪验证

化学位点—方向—连续多尺度相互作用图不是四篇论文中可直接复制的现成模块，而是本项目根据肽化学提出的组合式结构假说。项目先证伪了单中心、多锚点和固定 6/12 Å 双图，再用芳香/极性位点拆分、脂肪族疏水覆盖、方向不变量、双距离 RBF、显式相互作用类型和冻结基线零初始化残差进行优化。MCC-only checkpoint 暴露了“分类提升但排序均值下降”的目标错配；Pareto 规则把五折 MCC 从 0.8312 提高到 0.8342、AUPRC 从 0.9388 提高到 0.9401，且两者均为 5/5 折正向。同残基位点边在不改变阈值指标的情况下又带来平均 AUPRC `+0.00003`。这一创新体现为**问题提出、可运行形式化、全面组件证伪和跨折有限正证据**，而不是已经证明的独立数据排序领先。
...
AUROC/AUPRC是阈值无关的排序指标；BACC、MCC、F1、Precision和Recall都依赖阈值。不同实验阈值不同，不能只抄指标而忽略阈值来源。当前3D-v2默认阈值为`0.677819`，历史3D-v1回退阈值为`0.85`，1D为`0.50`，2D为`0.25`。
...
- 3D-v2在pLDDT约束下联合利用Cα空间邻域与全原子化学位点，已形成当前默认线路；
...
当前项目已经完成从一维序列基线到2D原子图、3D几何、多通路可靠度、严格nested验证、ToxinPred3冻结外部验证以及全原子化学位点组件消融的完整探索。当前默认3D-v2由历史3D-v1主干和全原子化学位点残差组成；历史3D-v1在缺少化学位点输入时自动回退。一维BiLSTM在现有严格外部集上迁移最好，是重要的无结构备份；二维原子图作为互补消融；动态路由保留为可靠度分析；残差专家、RankNet、选择性风险学习器和conformal均因未通过对应门槛而停止。3D-v2的训练内五折增益支持工程默认升级，但尚无新的独立数据证明全面泛化优势。

...
更新时间：2026-07-17
状态：默认模型为3D-v2；现有数据上的交叉验证、组件消融、配对统计、分层分析和案例解释已完成
...
当前默认分类器是 **ESM2 + 序列 Transformer + pLDDT-aware Cα-EGNN + 全原子化学位点交互残差 + learned-confidence fusion** 的3D-v2。这个名称中的“3D”表示它使用三维结构，并不意味着模型只有结构输入；它同时保留了完整的1D序列分支。训练内五折结果显示，化学位点支路相对匹配3D-v1对照带来稳定但幅度较小的排序改善；Test1略有提升，Test2则表现为AUPRC提高、MCC/F1下降。因此，当前更准确的结论是“化学位点有助于部分样本的概率排序”，而不是“3D-v2在所有数据域上都优于旧模型”。
...
这张表也说明了为什么项目保留多条线路：Test1更支持当前1D+3D组合，Test2上则由不同线路分别取得最高AUROC、MCC/F1和AUPRC。它们是互补视角，而不是一条线路在所有指标上统一胜出。
...
本项目的贡献不在于重新发明ESM2、EGNN、原子图或跨注意力，而在于把四篇参考工作中分散的序列、原子化学、等变几何和多尺度建模思路，组织成一个能够显式处理结构置信度、能够回退、也能被消融检验的框架。其中，“多化学位点—方向—连续双尺度相互作用残差”是项目在这些基础上提出的具体结构假说。
...
四篇论文在项目中的作用并不对称。ToxMSRC提供相同数据背景和直接比较基准；PeptiTox和StrucToxNet主要影响当前默认的序列—结构主线；ToxiPep主要启发了2D原子图与残基对齐实验。由于2D并未稳定提高默认决策性能，ToxiPep的思路最终体现为对照和解释能力，而不是直接组成默认分类器。项目真正新增的部分，是将这些思路统一到pLDDT感知的残基级框架中，并以全原子化学位点连续双尺度残差补充Cα图对侧链化学的粗糙表示。

...
这张表说明新论文路线带来的主要变化：真实ProtT5预训练均值表征、折内边界增强和多随机种子平均共同把序列线路从“回退模型”推进为当前同数据性能候选，并在两套历史测试上同时提高MCC和排序指标。进一步保留残基位置的ProtT5局部CNN没有形成额外泛化收益，说明“更细粒度”不等于“更强预测”；当前数据更支持全局pooled语义。3D-v2仍保留为结构创新与生物学解释主线，因为序列候选没有显式空间或化学位点输入，且尚无新的外部迁移证据。
